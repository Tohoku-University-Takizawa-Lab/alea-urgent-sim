...
public class ResourceUrgentQueueUtilLogger implements JobResourceInfoLogger {
	...
	public void init(Map<String, String> config) {
		...
		try {
			...
			// Print header
			outputWriter.append("hour,numBusyPE,numFreePE,utilization,queueSize,urgentQueueSize\n");
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	@Override
	public void logResources(double clock, List<ResourceInfo> infos, long queueSize, long urgentQueueSize) {
		...
		if ((numBusy + numFree) > 0) {		// This is a workaround when GridSim return 0 for both counters
			...
			try {
				outputWriter.append(hour + "," + numBusy + "," + numFree + "," 
						+ String.format("%.2f", utilization) + "," + queueSize + "," + urgentQueueSize + "\n");
				outputWriter.flush();
			} catch (IOException e) {
				e.printStackTrace();
			}
