...
/**
 * Class AdvancedSpaceShared (version 1.0)<p> This class is an allocation policy
 * for GridResource that behaves exactly like First Come First Serve (FCFS).
 * This is a basic and simple scheduler that runs each Gridlet to one or more
 * Processing Element (PE). If a Gridlet requires more than one PE, then this
 * scheduler wait till these PEs are available. If the gridlet requires more PEs
 * than is available in the whole resource the gridlet is returned as FAILED. In
 * this version <b>simple co-allocation</b> is allowed such that parallel jobs
 * may be executed on one or more machines if necessary (not enough PEs is
 * available on single machine).<p> Moreover, it allows to simulate specific
 * machine's failures, killing (and returning to the Scheduler) such jobs at
 * such machine.
 *
 ...
 */
public class AdvancedSpaceSharedWithRAM extends AllocPolicy {

    public ResGridletList gridletQueueList_;     // Queue list
    public ResGridletList gridletInExecList_;    // Execution list
    public ResGridletList gridletPausedList_;    // Pause list
     ...
     * String) @post $none @pre resourceName != null @pre entityName != null
     ...
     * Gridlets can't be processed. A GridResource must contain one or more
     * Machines. A Machine must contain one or more PEs. </ul>
     */
    AdvancedSpaceSharedWithRAM(String resourceName, String entityName, ComplexResourceCharacteristics resConfig) throws Exception {
    ...
    /**
     * Handles internal events that are coming to this entity. @pre $none @post
     * $none
     */
    public void body() {
     ...
     * whether this operation is success or not, <tt>false</tt> otherwise (don't
     * care) @pre gl != null @post $none
     */
    public void gridletSubmit(Gridlet gl, boolean ack) {
        ...
        //System.out.println(rgl.getGridletID() + " allocating... queued = "+gridletQueueList_.size()+" cpu="+numPE+" ppn="+((ComplexGridlet) gl).getPpn()+" nodes="+((ComplexGridlet) gl).getNumNodes());
        ...
        if (numPE > 1) {
            ...
            if (numPE <= freePE && gridletQueueList_.size() == 0) {
                ...
                //System.out.println(rgl.getGridletID() + " allocating... with status = "+success);
            } else {
        ...
        if (success == false && failure == false) {
            ...
            System.out.println(rgl.getGridletID() + " = Gridlet QUEUED because free:" + this.getNumFreePE() + " while reqested:" + rgl.getNumPE() + " or " + gl.getNumPE() + " ");
     ...
     * @see gridsim.Gridlet @pre gridletId > 0 @pre userId > 0 @post $none
     */
     ...
     * might have the same Gridlet ID in the lists. <b>NOTE:</b> <ul> <li>
     * Before canceling a Gridlet, this method updates all the Gridlets in the
     * execution list. If the Gridlet has no more MIs to be executed, then it is
     * considered to be <tt>finished</tt>. Hence, the Gridlet can't be canceled.
     *
     ...
     *
     * @param gridletId a Gridlet ID
     * @param userId the user or owner's ID of this Gridlet @pre gridletId > 0
     * @pre userId > 0 @post $none
     */
     ...
     * @param ack an acknowledgement, i.e. <tt>true</tt> if wanted to know
     * whether this operation is success or not, <tt>false</tt> otherwise (don't
     * care) @pre gridletId > 0 @pre userId > 0 @post $none
     */
     ...
     * important as many Users might have the same Gridlet ID in the lists. <p>
     ...
     * @param ack an acknowledgement, i.e. <tt>true</tt> if wanted to know
     * whether this operation is success or not, <tt>false</tt> otherwise (don't
     * care) @pre gridletId > 0 @pre userId > 0 @pre destId > 0 @post $none
     */
     ...
     * (CPUs) while queued gridlets require less. @pre $none @post $none
     */
     ...
     * the current time. @pre $none @post $none
     */
    private void updateGridletProcessing() {
        ...
        /*
         * Iterator iter = gridletInExecList_.iterator(); while ( iter.hasNext()
         * ) { obj = (ResGridlet) iter.next();
         *
         * // Updates the Gridlet length that is currently being executed load
         * = getMIShare( timeSpan, obj.getMachineID());
     ...
     * <tt>timeSpan</tt> @pre timeSpan >= 0.0 @pre machineId > 0 @post $result
     * >= 0.0
     */
     ...
     * <tt>false</tt> otherwise @pre rgl != null @post $none
     */
    ...
    private boolean allocatePEtoGridlet(ResGridlet rgl, int numPE) {
        ...
        long ram = ((ComplexGridlet) rgl.getGridlet()).getRam();
    ...
    /**
     * Forecast finish time of a Gridlet. <tt>Finish time = length / available
     * rating</tt>
     *
     * @param availableRating the shared MIPS rating for all Gridlets
     * @param length remaining Gridlet length
     * @return Gridlet's finish time. @pre availableRating >= 0.0 @pre length >=
     * 0.0 @post $none
     */
    ...
    private void checkGridletCompletion() {
        ...
        /*
         * if(!removed){ System.out.println("POZOR nic neodstraneno a bezi:
         * "+gridletInExecList_.size());
        }
         */
    }

    private void checkGridletCompletion2() {
        ResGridlet obj = null;
        int i = 0;
        boolean removed = false;
        int rem = 0;
        ...
        while (i < gridletInExecList_.size()) {
            ...
            if (obj.getRemainingGridletLength() == 0.0) {
                ...
                gridletInExecList_.remove(obj);
                //System.out.println(obj.getGridletID()+" Ukoncuji... v case = "+Math.round(GridSim.clock())+" on "+GridSim.getEntityName(this.myId_)); 
                gridletFinish(obj, Gridlet.SUCCESS);
                removed = true;
                rem++;
                continue;
     ...
     *
     * @param rgl a ResGridlet object
     * @param status the Gridlet status @pre rgl != null @pre status >= 0 @post
     * $none
     */
    private void gridletFinish(ResGridlet rgl, int status) {
        ...
        if (rgl.getNumPE() > 1) {
            ...
            MachineWithRAM mr = null;
            for (int i = 0; i < machines.length; i++) {
                MachineWithRAM mr_2 = (MachineWithRAM) machinesList.getMachine(machines[i]);
                if (!mr_2.equals(mr)) {
                    mr = mr_2;
                    //System.out.println(mr.getPercUsedRam()+"% prev");
                ...
                //System.out.print(machines[i]+":"+pes[i]+" | ");
            ...
            MachineWithRAM mr = (MachineWithRAM) machinesList.getMachine(rgl.getMachineID());
     ...
     * found @pre gridletId > 0 @pre userId > 0 @post $none
     */
     ...
     * gi.getNumNodes();
     *
     * MachineList machines = resource_.getMachineList(); int allocateNodes =
     * numNodes;
     *
     * for (int i = 0; i < machines.size(); i++) { MachineWithRAM machine =
     * (MachineWithRAM) machines.get(i); // cannot use such machine if
     * (machine.getFailed()) { continue; } if (machine.getNumFreePE() >= ppn) {
     * allocateNodes --; } if (allocateNodes <= 0) { return true; } } // all
     * machines tested and not enough CPUs found to execute job now return
     * false; }
     */
} // end class
